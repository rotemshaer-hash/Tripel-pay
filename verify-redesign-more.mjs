...
const shots = "/tmp/claude-0/-home-user-Telegram-rs/4f8aa2eb-e0e2-5755-84ac-b0e801e09c28/scratchpad/redesign-shots2";
...
const email = `dana.redesign2.${Date.now()}@example.com`;
...
await page.getByRole("link", { name: "הישגים" }).click();
await page.waitForTimeout(300);
await shot("03-child-achievements");

await page.getByRole("link", { name: "חיסכון" }).click();
await page.waitForTimeout(300);
await shot("04-child-savings");

await page.getByRole("link", { name: "הידע שלי" }).click();
await page.waitForTimeout(300);
await shot("05-child-courses");

await page.getByLabel("שליחת כסף").click().catch(() => {});
await page.waitForTimeout(300);
await shot("06-send-money-modal");
