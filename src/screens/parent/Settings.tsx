...
export function ParentSettings() {
  ...
  return (
    <div className="screen">
      ...
      <SectionTitle>כרטיס וארנק</SectionTitle>
      <div style={{ padding: "0 20px" }}>
        <Card style={{ background: s.frozen ? "var(--coral-100)" : undefined }}>
          <Row label={s.frozen ? "הכרטיס מוקפא" : "הקפאת כרטיס"} hint="חוסם מיידית כל תשלום, העברה ומימוש מתנות של הילד" important>
            <Toggle
              checked={s.frozen}
              onChange={(v) => {
                patch({ frozen: v });
                showToast(v ? `הכרטיס של ${child.name} הוקפא` : `הכרטיס של ${child.name} שוחרר`);
              }}
            />
          </Row>
        </Card>
      ...
      <SectionTitle>הרשאות שימוש</SectionTitle>
      <div style={{ padding: "0 20px" }}>
        <Card>
          <Row label="מצלמה">
            <Toggle checked={s.cameraPermission} onChange={(v) => patch({ cameraPermission: v })} />
          </Row>
          <Row label="טביעת אצבע">
            <Toggle checked={s.fingerprintPermission} onChange={(v) => patch({ fingerprintPermission: v })} />
          </Row>
          <Row label="מיקום">
            <Toggle checked={s.locationPermission} onChange={(v) => patch({ locationPermission: v })} />
          </Row>
        </Card>
      </div>

      <SectionTitle>הרשאות תשלום בחנות</SectionTitle>
      <div style={{ padding: "0 20px" }}>
        <Card>
          <div style={{ display: "flex", gap: 8, padding: "10px 0 12px" }}>
            {(["faceid", "fingerprint", "pin"] as const).map((m) => (
              <button
                key={m}
                onClick={() => patch({ storeAuthMethod: m })}
                style={{
                  flex: 1,
                  padding: "9px 4px",
                  borderRadius: 10,
                  border: s.storeAuthMethod === m ? "2px solid var(--teal-500)" : "1px solid var(--line)",
                  background: s.storeAuthMethod === m ? "var(--teal-100)" : "transparent",
                  fontSize: 12,
                  fontWeight: 700,
                  cursor: "pointer",
                }}
              >
                {m === "faceid" ? "זיהוי פנים" : m === "fingerprint" ? "טביעת אצבע" : "קוד"}
              </button>
            ))}
          </div>
        </Card>
      ...
      <SectionTitle>הגדרות תשלום בחנות</SectionTitle>
      <div style={{ padding: "0 20px" }}>
        <Card>
          <Row label="הגבלת תשלום לרכישה בודדת (₪)" hint="הסכום המרבי שהילד יכול להוציא בסריקת QR אחת" important>
            <input
              type="number"
              value={s.paymentLimit}
              onChange={(e) => patch({ paymentLimit: Number(e.target.value) })}
              style={{ width: 70, padding: "6px 8px", borderRadius: 8, border: "1px solid var(--line)", textAlign: "center" }}
            />
          </Row>
          <Row label="התראות על תשלום">
            <Toggle checked={s.paymentNotifications} onChange={(v) => patch({ paymentNotifications: v })} />
          </Row>
        </Card>
      </div>
