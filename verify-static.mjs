...
await page.goto("http://localhost:8790/triple-pay-app.html");
...
await page.getByPlaceholder("לדוגמה: דנה").fill("דנה");
await page.getByText("המשך").click();

await page.waitForURL("**/onboarding/biometric");
await shot("06-biometric");
await page.getByText("סריקת טביעת אצבע").click();

await page.waitForURL("**/onboarding/success", { timeout: 3000 });
await shot("07-success");
await page.getByText("למסך הבית").click();
...
await page.locator('nav a[href$="/parent/settings"]').click();
